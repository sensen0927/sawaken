...
<div id="content" class="clearfix">
    ...
    <article>
        ...
        <section <?php post_class(); //投稿の種類に応じたクラスを付加 
                            ?>>
            <h1 class="blog-title-article"><?php the_title(); //投稿（固定ページ）のタイトルを表示 
                                                    ?></h1>
            <div class="blog-wrap">
                <div class="blog-header">
                    <div class="date"><?php the_time('Y.m.d'); //投稿日時を表示 パラメータで書式を指定 
                                                ?></div>
                    <div class="category">カテゴリー：<?php the_category(','); //投稿の属するカテゴリー名をすべて表示 パラメータで区切り文字を指定 
                                                        ?></div>
                </div>
                <div class="blog-body">
        ...
        <div id="comment-container"><?php comments_template(); //コメントテンプレートを取得 
                                            ?></div>
